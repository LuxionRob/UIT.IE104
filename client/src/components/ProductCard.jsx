...
const ProductCard = ({ product, className }) => {
  return (
    <Link to={`/products/${product.id}`} className={`flex w-full flex-col hover:shadow-2xl ${className}`}>
      ...
      <span data-text={product.name} className='tooltip px-3 pt-2 hover:text-primary'>
      ...
      <div className='mt-4 flex items-center justify-between px-3 pb-3 text-primary lg:mt-2 lg:flex-col lg:px-0 sm:mt-1 sm:px-0'>
        <h2 className='text-3xl lg:self-start lg:text-left lg:text-xl'>{transformToVNCurrency(product.price)}</h2>

        <button
          className='button border-primary p-1 text-primary hover:bg-primary hover:text-white lg:mt-2 lg:w-full'
          to='/payment'
        >
          Đặt hàng
        </button>
